...
export default defineApiEventHandler(async (event: H3Event<{ query: z.input<typeof zParameter> }>) => {
    ...
    const list = await db
        .select({
            ...
            date: tNews.date,
            keywords: sql<
                Array<NewsKeywordItem>
            >`json_agg(json_build_object('keyword', ${tNewsEffect.keyword}, 'effect', ${tNewsEffect.effect}, 'confidence', ${tNewsEffect.confidence}, 'reason', ${tNewsEffect.reason}))`,
        })
        .from(tNews)
