...
export default defineApiEventHandler(
    async (
        ...
        const t = db
            ...
            .where(
                query.search
                    ? or(ilike(tStock.name, `%${query.search}%`), ilike(tStock.symbol, `%${query.search}%`))
                    : undefined,
            )
            .as("t");
        ...
        const stocks = await db
            .select()
            .from(t)
            .orderBy(sql`(ln(${t.news_count} + 1) * ln(coalesce(${t.turnover}, 0) + 1)) DESC NULLS LAST`, desc(t.turnover))
            .limit(query.size);
